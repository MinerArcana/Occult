...
public abstract class MultiblockControllerBase implements IMultiblockValidator {
	...
	public boolean shouldConsume(MultiblockControllerBase otherController) {
		...
		if (res < 0) {
			...
			if (res < 0) {
				...
				Occult.LOGGER.fatal(String.format("My Controller (%d): size (%d), parts: %s", hashCode(),
						connectedParts.size(), getPartsListString()));
				Occult.LOGGER.fatal(String.format("Other Controller (%d): size (%d), coords: %s", otherController.hashCode(),
	...
	private void auditParts() {
		...
		Occult.LOGGER.warn(String.format("[%s] Controller found %d dead parts during an audit, %d parts remain attached",
