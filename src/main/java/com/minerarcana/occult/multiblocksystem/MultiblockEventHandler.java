...
public final class MultiblockEventHandler {
	...
	public void onChunkLoad(final ChunkEvent.Load loadEvent) {
		...
		MultiblockRegistry.INSTANCE.onChunkLoaded(loadEvent.getWorld().getWorld(), chunk.getPos().x, chunk.getPos().z);
	}

	@SubscribeEvent(priority = EventPriority.NORMAL)
	public void onWorldUnload(final WorldEvent.Unload unloadWorldEvent) {
		MultiblockRegistry.INSTANCE.onWorldUnloaded(unloadWorldEvent.getWorld().getWorld());
	}
