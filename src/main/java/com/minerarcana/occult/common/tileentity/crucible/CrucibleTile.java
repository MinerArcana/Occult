...
public class CrucibleTile extends TileEntity implements ISidedInventory, IRecipeHolder, IRecipeHelperPopulator, ITickableTileEntity
{
    ...
    public int setFuelTemp(World world, BlockState state, BlockPos pos) {
        ...
        if(world.getBlockState(pos1).getBlock().isIn(OccultTags.Blocks.MEDIUMHEATSOURCE))
        {
            FuelTemp = 1000;
        }
        if(world.getBlockState(pos1).getBlock().isIn(OccultTags.Blocks.LOWHEATSOURCE))
        {
            FuelTemp = 250;
        }
        if(world.getBlockState(pos1).getBlock().isIn(OccultTags.Blocks.MEDIUMLOWHEATSOURCE))
        {
            FuelTemp = 650;
        }
        if(world.getBlockState(pos1).getBlock().isIn(OccultTags.Blocks.LOWHIGHHEATSOURCE))
        {
            FuelTemp = 1500;
        }
        if(world.getBlockState(pos1).getBlock().isIn(OccultTags.Blocks.HIGHHEATSOURCE))
        {
            FuelTemp = 2000;
        }
        if(world.getBlockState(pos1).getBlock().isIn(OccultTags.Blocks.EXTREMELYHIGHHEATSOURCE))
        {
            FuelTemp = 3000;
        }
    ...
    private int getMaxTemp() {
        return this.world.getRecipeManager().getRecipe(this.recipeType, this, this.world).map(CrucibleRecipes::getMaxtemp).orElse(200);
    }
